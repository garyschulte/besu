      ...
      try {
        ...
        while (!entriesToDelete.isEmpty()) {
          if(storageToDelete==null){
            storageToDelete = worldStateUpdater.getStorageToUpdate().computeIfAbsent(address,
                add ->new StorageConsumingMap<>(address, new ConcurrentHashMap<>(), worldStateUpdater.getStoragePreloader()));
          }
          for (Map.Entry<Bytes32,Bytes> slot :entriesToDelete.entrySet()) {
            final StorageSlotKey storageSlotKey =
                new StorageSlotKey(Hash.wrap(slot.getKey()), Optional.empty());
            final UInt256 slotValue = UInt256.fromBytes(Bytes32.leftPad(RLP.decodeValue(slot.getValue())));
            maybeStateUpdater.ifPresent(
                bonsaiUpdater ->
