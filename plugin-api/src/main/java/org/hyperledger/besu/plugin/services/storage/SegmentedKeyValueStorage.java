...
public interface SegmentedKeyValueStorage extends Closeable {
   ...
   * #get(SegmentIdentifier, byte[])} and wraps it with {@link MemorySegment#ofArray(byte[])}. A
   * native-FFM storage plugin may override this to avoid the heap copy.
   *
   * @param <T> the type returned by the reader
   * @param segment the storage segment
